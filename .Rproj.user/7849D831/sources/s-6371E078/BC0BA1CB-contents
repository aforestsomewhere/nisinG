...
###################################
# Modified version for main paper #
###################################
# just one legend
#remove T_24 exposure from titles
###############################
# Robust Aitchisons           #
###############################
cmd_res = cmdscale(dist_mat_ai, 
...
p2 = ggplot(pcoa_meta,aes(x = PC1, y = PC2, color=strep_exposure)) + 
  ...
  scale_size_continuous(limits=c(2,2))+
  scale_shape_manual(name="Treatment",
                     labels=c("Control", #b
                              expression(paste(italic("F. nucleatum"), " DSM15643")), #f
                              expression(paste(italic("F. nuc"), " DSM15643 + ", italic("S. sal"), " DPC6487")), #fs
  ...
  guides(fill= "none",
         col= "none",
         shape = "none") +
  ggtitle("Robust Aitchison Distance")
p2
p2a <- p2 + expand_limits(x = c(-15,15), y = c(-10,13))
p2a
...
p4
p4a <- p4 + expand_limits(x=c(-0.2,.2), y = c(-.1,.1))
p4a
...
p5a <- p5 + expand_limits(x = c(-.24,.44), y = c(-.095,.095))
p5a
...
#expand axes
p6a <- p6 + expand_limits(x=c(-.15, .15), y = c(-.0003,.0003))
p6a

#combine

p0 <- ggdraw(plot_grid(p2a,p4a, p5a, p6a,
                       align = "h", ncol = 2,
                       labels = "AUTO"))
p0
p0a <- ggdraw(plot_grid(p0, legend_1,
                        nrow=2, rel_heights=c(10, 2),
                        rel_widths=c(1, 2),
                        axis = 'l', align = 'h'))
p0a

ggsave("figures/beta_T_24_allmetrics_main.png", dpi=600)
ggsave("figures/beta_T_24.pdf", width=30, height=40, dpi=300, units="cm")

################################################
# Abundances at T_24
#ComplexHeatmap::Heatmap at T24
mat <- as.matrix(mpa_table_filtered)
