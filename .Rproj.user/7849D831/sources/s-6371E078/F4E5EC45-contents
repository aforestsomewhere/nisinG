#' Script to calculate dominance on NisinG samples
#' Developed by A. Kate Falà (https://github.com/aforestsomewhere)
#' TESTING FOR DIFFERENCE BY TREATMENT OR TIMEPOINT or EXPOSURE
...
#richness
richness <- plot_layer(plot_base(alpha_values, metric="richness", group = "timepoint"),
                       calc_wilcox(alphas = alpha_values, metric = "richness", group="treatment"))
richness
alpha_all <-rbind(alpha_all, compute_stats(calc_wilcox(alphas = alpha_values, metric = "richness", group="timepoint")))
#shannon
