#final alpha T_24
#Only testing of strep exposure Y/N
#remove sig. testing from plot_layer as it returns Wilcoxon values, and all are n.s.
...
kruskal_results <- results_strep

############################################
### Follow up test sig. from KW          ###
### No significant differences           ###
### Run wilcoxon regardless for plotting ###
############################################
...
for (i in 1:length(indices)){
  alpha_time <-rbind(alpha_time, calc_wilcox(alphas = alpha_values, metric = indices[i], group="strep_exposure"))
...
################################################
#function with sig
legend1 <- get_legend(plot_leg(plot_base(alpha_values, metric="richness", group = "strep_exposure"), legendtitle = "Group", pal=palette_collapsed) +
                        scale_fill_manual(name="Group",
                                          labels=c(expression(paste("No ", italic("S. salivarius"), " DPC6487")), expression(paste("Any ", italic("S. salivarius"), " DPC6487"))),
                                          values = palette_collapsed))
  

richness <- plot_layer(plot_base(alpha_values, metric="richness", group = "strep_exposure"),
                         calc_wilcox(alphas = alpha_values, metric = "richness", group="strep_exposure"),
                         legendtitle = "strep_exposure", pal=palette_collapsed) + xlab("Richness \n(No. observed species)") + ylab("")+
  theme(axis.text.x=element_blank(),
        axis.title = element_text(face="bold"))
richness
#shannon
shannon <- plot_layer(plot_base(alpha_values, metric="shannon", group = "strep_exposure"),
                       calc_wilcox(alphas = alpha_values, metric = "shannon", group="strep_exposure"),
                       legendtitle = "strep_exposure", pal=palette_collapsed) + xlab("Shannon's \n Diversity Index") + ylab("")+
  theme(axis.text.x=element_blank(),
        axis.title = element_text(face="bold"))
shannon
#inv_simpson
invsimpson <- plot_layer(plot_base(alpha_values, metric="inv_simpson", group = "strep_exposure"),
                      calc_wilcox(alphas = alpha_values, metric = "inv_simpson", group="strep_exposure"),
                      legendtitle = "strep_exposure", pal=palette_collapsed) + xlab("Inverse Simpson's \n Diversity Index") + ylab("")+
  theme(axis.text.x=element_blank(),
        axis.title = element_text(face="bold"))
invsimpson

#plot together
plots <- plot_grid(richness,shannon,invsimpson, nrow=1, align='v',  
                   label_size = 14)
plots
#make title
title <- ggdraw() + draw_label("Alpha Diversity", fontface='bold')
plots2 <- plot_grid(title, plots, ncol=1, rel_heights=c(0.1, 1)) 
#combine wth legend pulled from base plot
#leggo <- get_legend(plot_leg(richness))
#legend <- get_legend(plot_base(alpha_values, metric="richness", group = "treatment"))
p_all <- ggdraw(plot_grid(plots2, legend1, 
                          nrow=2, align='v', rel_heights=c(1, 0.1),
                          rel_widths=c(1, 2))) + 
  theme(plot.background = element_rect(fill="#FFFFFF", color = NA),
        axis.title.y = element_text(size=12, face="bold", colour = "black"))
p_all
ggsave("figures\\alpha_violins_T_24_exposure.pdf", p_all, height = 12, width = 20, units = "cm", dpi = 600)
ggsave("figures\\alpha_violins_T_24_exposure.png", p_all, dpi=600)
