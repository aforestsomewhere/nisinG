...
filter_fit <- function(maaslin_out){
  maaslin_out %>%
...
# mpa_table[,colnames(mpa_table) %in% rownames(meta0)] -> mpa_table0
# #T_6
# meta6 <- meta %>% filter(timepoint=="T_6") %>% select(treatment)
# mpa_table[,colnames(mpa_table) %in% rownames(meta6)] -> mpa_table6
# #T_24
# meta24 <- meta %>% filter(timepoint=="T_24") %>% select(treatment)
# mpa_table[,colnames(mpa_table) %in% rownames(meta24)] -> mpa_table24
...
### t_0          ###
####################
run_maaslin <- function(mpa, meta ,category, group, time){
  #variables
  outputdir <- paste("maaslin2_collapsed/",time,"/",category,"_",group,sep="")
...
# #for each timepoint
# #T_0
# t0_B <- filter_fit(run_maaslin(mpa=mpa_table0, meta=meta0, category="treatment", group="B", time="t0"))
...
# #T_6
# t6_B <- filter_fit(run_maaslin(mpa=mpa_table6, meta=meta6, category="treatment", group="B", time="t6"))
...
# #T_24
# t24_B <- filter_fit(run_maaslin(mpa=mpa_table24, meta=meta24, category="treatment", group="B", time="t24"))
...
mpa_table[,colnames(mpa_table) %in% rownames(meta24)] -> mpa_table24
#T_0
t0_Y_ex <- run_maaslin(mpa=mpa_table0, meta=meta0, category="strep_exposure", group="Y", time="t0")
t0_Y_ex <- t0_Y_ex$results
filter_fit(t0_Y_ex)
#T_6
t6_Y_ex <- run_maaslin(mpa=mpa_table6, meta=meta6, category="strep_exposure", group="Y", time="t6")
t6_Y_ex <- t6_Y_ex$results
filter_fit(t6_Y_ex)
#T_24
t24_Y_ex <- run_maaslin(mpa=mpa_table24, meta=meta24, category="strep_exposure", group="Y", time="t24")
t24_Y_ex <- t24_Y_ex$results
filter_fit(t24_Y_ex)
