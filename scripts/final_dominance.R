...
#DBP and Gini
#function with sig
legend1 <- get_legend(plot_leg(plot_base(dom_24, metric="dbp", group = "strep_exposure"), legendtitle = "Group", pal=palette_collapsed) +
...
#dbp
dbp <- plot_layer(plot_base(dom_24, metric="dbp", group = "strep_exposure"),
                       calc_wilcox(dom = dom_24, metric = "dbp", group="strep_exposure"),
                       legendtitle = "strep_exposure", pal=palette_collapsed) + xlab("Berger-Parker Index \n (Dominance)") + ylab("")+
  theme(axis.text.x=element_blank(),
        axis.title = element_text(face="bold"))
dbp
#gini
gini <- plot_base(dom_24, metric="gini", group = "strep_exposure") + 
  xlab("Gini Index \n (Dominance)") + ylab("")+
  scale_fill_manual(name="Group",
                    labels=c(expression(paste("No ", italic("S. salivarius"), " DPC6487")), expression(paste("Any ", italic("S. salivarius"), " DPC6487"))),
                    values = palette_collapsed) +
...
gini
#plot together
plots <- plot_grid(dbp, gini, gini, nrow=1, align='v',  
                   label_size = 14)
plots
#make title
title <- ggdraw() + draw_label("Dominance", fontface='bold')
plots2 <- plot_grid(title, plots, ncol=1, rel_heights=c(0.5, .56)) # to get similiar scale to alpha diversity plots 
...
p_all <- ggdraw(plot_grid(plots2, legend1, 
  ...
  theme(plot.background = element_rect(fill="#FFFFFF", color = NA),
        axis.title.y = element_text(size=12, face="bold", colour = "black"))
p_all
ggsave("figures\\dom_violins_T_24_exposure_dbp_gini.png", p_all, dpi=600)
########################
# abundance values     #
