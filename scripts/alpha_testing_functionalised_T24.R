...
calc_vegan_table <- function(mpa, metadata, time){
  ...
  alpha_values <- alpha_values %>% 
    dplyr::select(sample_name, sample_id, treatment, replicate, treatment_replicate, timepoint, exp_group, richness, shannon, simpson, inv_simpson) %>%
    filter(timepoint==time)
  return(alpha_values)
}

alpha_values <- calc_vegan_table(mpa_mat, meta, "T_24")
...
#simpson - present inverse instead
...
plots <- plot_grid(richness,shannon,invsimpson, nrow=1, align='v',  
                   label_size = 14)
#make title
title <- ggdraw() + draw_label("Alpha diversity between groups at T_24", fontface='bold')
...
p_all
#ggsave("figures\\alpha_violins_T_24.emf", width = 4, height = 4, dpi = 300, device = {function(filename, ...) devEMF::emf(file = filename, ...)})
ggsave("figures\\alpha_violins_T_24.png", p_all, dpi=600)
ggsave("figures\\alpha_violins_T_24.pdf", p_all, height = 8, width = 12, units = "in", dpi = 150)
#outputstats
alpha_all<- alpha_all[-1,]
write.csv(alpha_all, "reports\\alpha_tested_T_24.csv", row.names = FALSE)
