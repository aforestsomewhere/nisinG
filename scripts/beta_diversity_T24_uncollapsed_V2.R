...
### read data    ###
####################
#filtering out T_0 samples and adding column for any/no strep exposure
meta <- read.csv("cleaned_data/metadata.csv", nrows=36, row.names=1)
meta <- meta %>% filter(timepoint=="T_6") %>%
...
anosim_df_all <-rbind(anosim_df_all, calc_anosim(dist_mat_bc,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Bray-Curtis dissimilarity", groupname="Strep exposure"))
anosim_df_all[-1,]-> anosim_df_all #remove first shaper row (empty)
#Aitchisons
anosim_df_all <-rbind(anosim_df_all, calc_anosim(dist_mat_ai,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Robust Aitchisons", groupname="Strep exposure"))
#Weighted UNIFRAC
anosim_df_all <-rbind(anosim_df_all, calc_anosim(dist_mat_wUF,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Weighted UNIFRAC distances", groupname="Strep exposure"))
#Unweighted UNIFRAC
anosim_df_all <-rbind(anosim_df_all, calc_anosim(dist_mat_uUF,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Unweighted UNIFRAC distances", groupname="Strep exposure"))
...
#BC
anosim_df_all <-rbind(anosim_df_all, calc_adonis(dist_mat_bc,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Bray-Curtis dissimilarity", groupname="Strep exposure"))
#Aitchisons
anosim_df_all <-rbind(anosim_df_all, calc_adonis(dist_mat_ai,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Robust Aitchisons", groupname="Strep exposure"))
#Weighted UNIFRAC
anosim_df_all <-rbind(anosim_df_all, calc_adonis(dist_mat_wUF,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Weighted UNIFRAC distances", groupname="Strep exposure"))
#Unweighted UNIFRAC
anosim_df_all <-rbind(anosim_df_all, calc_adonis(dist_mat_uUF,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Unweighted UNIFRAC distances", groupname="Strep exposure"))
...
#BC
anosim_df_all <-rbind(anosim_df_all, calc_disp(dist_mat_bc,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Bray-Curtis dissimilarity", groupname="Strep exposure"))
#Aitchisons
anosim_df_all <-rbind(anosim_df_all, calc_disp(dist_mat_ai,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Robust Aitchisons", groupname="Strep exposure"))
#Weighted UNIFRAC
anosim_df_all <-rbind(anosim_df_all, calc_disp(dist_mat_wUF,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Weighted UNIFRAC distances", groupname="Strep exposure"))
#Unweighted UNIFRAC
anosim_df_all <-rbind(anosim_df_all, calc_disp(dist_mat_uUF,group=meta$strep_exposure, dataset="MetaPhlAn4", metric="Unweighted UNIFRAC distances", groupname="Strep exposure"))
write.csv(anosim_df_all, file="reports\\beta_testing_T24_collapsed.csv")
#############
# Plots     #
#############
#functions to amend axis names for ordinations in phyloseq
#test$children[[1]]$label
...
uUF.ordu = ordinate(phy, method="PCoA", distance="unifrac", weighted=FALSE)

#base plot for legend
leggo <- plot_ordination(phy, uUF.ordu, type="sites", color="strep_exposure") +
  geom_mark_ellipse(aes(color = strep_exposure)) +
  ...
  geom_point(aes(shape=treatment, fill=strep_exposure), size=3,colour="black") +
  ...
  guides(#color= guide_legend(nrow=2, byrow=TRUE),
    color= "none",
    shape = guide_legend(override.aes=list(fill=c("#FF0000", "#FF0000","#00A08A","#00A08A")),
                         nrow=4, byrow=TRUE),
    fill = guide_legend(override.aes=list(shape=21))) +
  ggtitle("Unweighted UniFrac, nMDS, unnormalised, MPA4")
plegend <- get_legend(leggo)
#unweighted UNIFRAC
p0 <- plot_ordination(phy, uUF.ordu, type="sites", color="strep_exposure") +
  geom_mark_ellipse(aes(color = strep_exposure)) +
  scale_color_manual(name="Inoculation",
                     labels=c(expression(paste("No ",italic("S. salivarius"))),
                              expression(paste("Any ",italic("S. salivarius")))), 
  ...
  guides(color = "none",
         shape = "none",
         fill = "none") +
  ggtitle("PCoA of unweighted UNIFRAC distances")
#adjust x and y limits
p0 <- p0 + expand_limits(x = c(-.45, .28), y = c(-.15, .12)) +
  xlab(new_xlab(p0)) + ylab(new_ylab(p0))

#weighted
wUF.ordu = ordinate(phy, method="PCoA", distance="unifrac", weighted=TRUE)
p1 <- plot_ordination(phy, wUF.ordu, type="sites", color="strep_exposure") +
  geom_mark_ellipse(aes(color = strep_exposure)) +
  scale_color_manual(name="Inoculation",
                     labels=c(expression(paste("No ",italic("S. salivarius"))),
                              expression(paste("Any ",italic("S. salivarius")))), 
...
p1
p1 <- p1 + expand_limits(y=c(-.04,0.06)) +
  xlab(new_xlab(p1)) + ylab(new_ylab(p1))
p1
#Bray Curtis
...
meta$strep_exposure <- factor(meta$strep_exposure)
...
p2 <- p2 + expand_limits(x=c(-.2,.2),y = c(-.08, .08))
p2
plots <- plot_grid(p2, p1, p0, plegend, labels = c('A','B','C',''))
plots
ggsave("figures/beta_T24_collapsed.pdf", width=29, height=21, dpi=300, units="cm")
