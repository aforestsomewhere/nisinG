...
legend <- ggviolin(alpha_values, 
  ...
  theme(legend.box = "horizontal", 
        ...
        legend.title = element_text(face="bold", size=20, face="bold", colour = "black"),
        axis.ticks.x = element_blank(),
        axis.text.x = element_blank(size=12, face="bold", colour = "black"))
...
plots <- plot_grid(p1,p2,p3,p4, nrow=1, align='v',  
                   label_size = 14)
#plots
p_all <- ggdraw(plot_grid(plots, legend, 
                 nrow=2, align='v', rel_heights=c(1, 0.2),
                 rel_widths=c(1, 2))) + 
  theme(plot.background = element_rect(fill="#FFFFFF", color = NA),
        axis.title.y = element_text(size=12, face="bold", colour = "black")) +
  ggtitle("T_24")
ggsave("figures\\alpha_violins_T24.emf", width = 4, height = 4, dpi = 300, device = {function(filename, ...) devEMF::emf(file = filename, ...)})
ggsave("figures\\alpha_violins_T24.png", p_all, dpi=600)
ggsave("figures\\alpha_violins_T24.pdf", p_all, height = 8, width = 12, units = "in", dpi = 150)
#outputstats
