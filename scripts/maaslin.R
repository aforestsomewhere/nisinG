...
run_maaslin <- function(mpa, meta ,category, group, time){
  #variables
  outputdir <- paste("maaslin2/",time,"/",category,"_",group,sep="")
...
mpa_table[,colnames(mpa_table) %in% rownames(meta24)] -> mpa_table24
#T_0
t0_Y_ex <- filter_fit(run_maaslin(mpa=mpa_table0, meta=meta0, category="strep_exposure", group="Y", time="t0"))

#T_6
t6_Y_ex <- filter_fit(run_maaslin(mpa=mpa_table6, meta=meta6, category="strep_exposure", group="Y", time="t6"))
#T_24
t24_Y_ex <- filter_fit(run_maaslin(mpa=mpa_table24, meta=meta24, category="strep_exposure", group="Y", time="t24"))
