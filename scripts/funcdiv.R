...
alpha_out <- alpha_div_contrib(metrics = metrics,
                               ...
                               abun_colname = "abundance")
#extract matrices
listOfDataframes <- lapply(alpha_out, function(x) as.data.frame(t(x)))

write.xlsx(listOfDataframes, "reports\func_div.xlsx")
...
png("figures/T24_Func_Heatmap.png",width=35,height=39,units="cm",res=1100)
draw(ht1)
dev.off()
