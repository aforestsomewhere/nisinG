...
sf2[,colnames(sf2) %in% rownames(meta0)] -> sf_t0
#T_6
meta6 <- meta %>% filter(timepoint=="T_6") %>% select(treatment)
sf2[,colnames(sf2) %in% rownames(meta6)] -> sf_t6
#T_24
meta24 <- meta %>% filter(timepoint=="T_24") %>% select(treatment)
sf2[,colnames(sf2) %in% rownames(meta24)] -> sf_t24
...
run_maaslin <- function(sf, meta ,category, group, time){
  #variables
  outputdir <- paste("maaslin2_func_collapsed/",time,"/",category,"_",group,sep="")
...
#for each timepoint
#T_0
t0_B <- filter_fit(run_maaslin(sf_t0, meta=meta0, category="treatment", group="B", time="t0"))
...
#T_6
t6_B <- filter_fit(run_maaslin(sf_t6, meta=meta6, category="treatment", group="B", time="t6"))
...
#T_24
t24_B <- filter_fit(run_maaslin(sf_t24, meta=meta24, category="treatment", group="B", time="t24"))
